...
export interface GameStats {
  gamesPlayed: number;
  winsByPlayerIndex: { 0: number; 1: number };
  draws: number;
}
...
export interface ServerToClientEvents {
  ...
  spectator_count_changed: (data: { spectatorCount: number }) => void;

  room_error: (data: { code: RoomErrorCode; message: string }) => void;
  action_error: (data: { code: ActionErrorCode; message: string }) => void;
  rematch_error: (data: { code: RematchErrorCode; message: string }) => void;

  /** Broadcast when one player votes for rematch (votes = 1) */
  rematch_requested: (data: { votes: number }) => void;
  ...
  all_stats: (data: { statsByGameId: Record<GameId, GameStats> }) => void;
